...
export const TripsPage = () => {
  ...
  const stats = useMemo(() => {
    const list: Row[] = [...trips]
      ...
      .map((t) => {
        ...
        return {
          trip: t,
          km,
          minutes: t.driving_time_minutes ?? 0,
          fuelL: t.fuel_consumed ?? 0,
          avgKmh: t.avg_speed ?? 0,
          tag: tagOf(km),
        };
      });
    ...
    const byTag = (tag: TagKind) => {
      ...
      return {
        count: sub.length,
        sumKm: sub.reduce((s, r) => s + r.km, 0),
        pct: list.length ? Math.round((sub.length / list.length) * 100) : 0,
      };
    };

    return {
      list, totalKm, totalMin, totalFuel, longest, avgKmh, consumption,
      byDistance: { corto: byTag('corto'), medio: byTag('medio'), largo: byTag('largo') },
    };
  }, [trips]);

  /* Origen más frecuente — actúa de hub en el mapa abstracto. */
  ...
  /* Selección por defecto → el viaje más largo (highlight del diseño). */
  useEffect(() => {
    if (!stats.longest) { setSelectedId(null); return; }
    setSelectedId((prev) =>
      prev && trips.some((t) => t.id === prev) ? prev : stats.longest!.trip.id);
  }, [trips]);
  ...
  /* Período del cuaderno — del viaje más reciente, o el mes actual. */
  ...
  /* Nodos del mapa — disposición radial alrededor del hub. */
  const mapNodes = stats.list.map((r, i) => {
    const angle = -Math.PI / 2 + (i * 2 * Math.PI) / Math.max(stats.list.length, 1);
    const x = HUB.x + 142 * Math.cos(angle);
    const y = HUB.y + 96 * Math.sin(angle);
    return { row: r, x, y, above: y <= HUB.y };
  });
  ...
  return (
    <>
      <div
        ...
        style={{ padding: '30px 24px', display: 'flex', flexDirection: 'column', gap: 24 }}
      >
        {/* ─── [1] Top strip ─────────────────────────────────────────────── */}
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center" style={{ gap: 14 }}>
            <span
              style={{
                width: 6, height: 6, borderRadius: 999, background: '#1cb05c',
                boxShadow: '0 0 0 4px rgba(28,176,92,.18)', display: 'inline-block',
              }}
            />
            <span
          ...
          <div className="flex items-center" style={{ gap: 8 }}>
            <button
              ...
              style={{
                border: '1px solid var(--color-silver-mist)', borderRadius: 999,
                padding: '8px 14px', fontWeight: 500, fontSize: 12,
                cursor: hasTrips ? 'pointer' : 'not-allowed',
              }}
            >
              ↓ CSV
            </button>
            <button
              ...
              style={{
                background: '#1d1d1f', color: '#fff', borderRadius: 999, border: 'none',
        ...
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] items-end" style={{ gap: 48 }}>
          ...
          <div>
            <span
              ...
              style={{ fontSize: 11, fontWeight: 500, letterSpacing: '.22em', color: '#0066cc' }}
            >
              ...
              {hasTrips ? (
                <>
                  <span className="text-ink" style={{ fontWeight: 500 }}>
                    {trips.length} {trips.length === 1 ? 'entrada' : 'entradas'}
                  </span>{' '}
                  ·{' '}
                  <span className="text-ink" style={{ fontWeight: 500 }}>
                    {nf0.format(stats.totalKm)} km
                  </span>{' '}
                  recorridos en {monthLower}.
                  {longest && <> El viaje más largo a {longest.trip.end_location}.</>}
                </>
              ) : (
                <>Aún no has registrado ningún viaje. Empieza a documentar tus rutas,
                  consumo y kilómetros.</>
              )}
            </p>
          </div>

          {/* 2b · Distribución por distancia */}
          <div
            className="bg-snow"
            style={{
              border: '1px solid var(--color-silver-mist)', borderRadius: 18,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 14,
                      padding: '10px 0',
                      borderTop: `1px solid ${i === 0 ? '#1d1d1f' : 'var(--color-silver-mist)'}`,
                    }}
                  >
                    <div className="flex items-center" style={{ gap: 8 }}>
                      <TripTag tag={tag} />
                    ...
                    <div
                      style={{
                        height: 8, background: '#f5f5f7', borderRadius: 999,
                        overflow: 'hidden',
                      }}
                    >
                      <div
                        style={{
                          ...
                          background: TAG_STYLE[tag].bar, borderRadius: 999,
                        }}
                      />
                    </div>
          ...
          {[
            {
              ...
              value: hasTrips ? nf0.format(stats.totalKm) : '–',
              unit: 'km',
            },
            {
              label: 'Viaje más largo',
              value: hasTrips && longest ? nf0.format(longest.km) : '–',
              unit: hasTrips && longest ? `km · ${longest.trip.end_location}` : 'km',
            },
            {
              label: 'Velocidad media',
              value: hasTrips && stats.totalMin > 0 ? nf0.format(stats.avgKmh) : '–',
              unit: 'km/h',
            },
            {
              label: 'Consumo medio',
              value: hasTrips && stats.consumption > 0 ? nf1.format(stats.consumption) : '–',
              unit: 'L / 100 km',
            },
          ].map((kpi) => (
            <div
            ...
            >
              ...
              <p style={{ marginTop: 12 }}>
                <span
                  ...
                  style={{
                    fontFamily: 'Inter, var(--font-sf-pro-display)',
                    fontWeight: 700, fontSize: 44, letterSpacing: '-0.9px',
                  }}
                >
                  {kpi.value}
                </span>
                <span style={{ fontSize: 13, color: '#707070', marginLeft: 6 }}>
                  {kpi.unit}
                </span>
              </p>
            </div>
          ...
          >
            <Route
              ...
              style={{
                ...
                color: '#1d1d1f', opacity: 0.04, pointerEvents: 'none',
              }}
            />
            ...
            <div className="flex items-center justify-between" style={{ position: 'relative' }}>
              ...
              <span
                ...
                style={{ fontSize: 10, letterSpacing: '.14em', color: '#c7c7cc' }}
              >
                Esc · No real
              </span>
            </div>
            ...
            <svg
              ...
              style={{ width: '100%', height: 290, marginTop: 14, position: 'relative' }}
            >
              {/* Líneas hub → destino */}
              {mapNodes.map((n) => {
                const isMax = longest?.trip.id === n.row.trip.id;
                return (
                  <line
                    key={`l-${n.row.trip.id}`}
                    x1={HUB.x} y1={HUB.y} x2={n.x} y2={n.y}
                    stroke={isMax ? '#b64400' : '#e0e0e5'}
              ...
              <circle
                ...
                stroke="#1d1d1f" strokeWidth={1} strokeDasharray="3 3"
              />
              <circle cx={HUB.x} cy={HUB.y} r={10} fill="#1d1d1f" />
              <text
                x={HUB.x} y={HUB.y + 42} textAnchor="middle"
                style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, fill: '#1d1d1f' }}
              >
                {hubName}
              </text>
              <text
                x={HUB.x} y={HUB.y + 56} textAnchor="middle"
                style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: '.14em', fill: '#a1a1a6' }}
              >
                ORIGEN
              </text>
              ...
              {mapNodes.map((n) => {
                const isMax = longest?.trip.id === n.row.trip.id;
                const isSel = selectedId === n.row.trip.id;
                const nameY = n.y + (n.above ? -18 : 24);
                const kmY   = n.y + (n.above ? -30 : 36);
                return (
                  <g
                    key={`n-${n.row.trip.id}`}
                    style={{ cursor: 'pointer' }}
                    onClick={() => setSelectedId(n.row.trip.id)}
                  >
                    {isSel && (
                      <circle
                        cx={n.x} cy={n.y} r={14} fill="none"
                        stroke={isMax ? '#b64400' : '#1d1d1f'} strokeDasharray="2 3"
                      />
                    )}
                    <circle
                      cx={n.x} cy={n.y}
                      r={isSel ? 8 : 5}
                      fill={isMax ? '#b64400' : '#707070'}
                    />
                    <text
                      x={n.x} y={nameY} textAnchor="middle"
                      style={{
                        fontFamily: 'var(--font-mono)', fontSize: 10,
                        fontWeight: isMax || isSel ? 700 : 500,
                        fill: isMax ? '#b64400' : '#474747',
                      }}
                    >
                      {n.row.trip.end_location}
                    </text>
                    <text
                      x={n.x} y={kmY} textAnchor="middle"
                      style={{
                        fontFamily: 'var(--font-mono)', fontSize: 9,
                        fill: isMax ? '#b64400' : '#a1a1a6',
                      }}
                    >
                      {nf0.format(n.row.km)} km
                    </text>
                  </g>
                );
              })}
            ...
            <div
              style={{
                marginTop: 'auto', borderTop: '1px solid var(--color-silver-mist)',
                paddingTop: 14,
              }}
            >
              <p style={{ fontSize: 12, lineHeight: 1.5, color: '#707070' }}>
                {hasTrips ? (
                  <>
                    <span className="text-ink" style={{ fontWeight: 500 }}>
                      {trips.length} {trips.length === 1 ? 'ruta' : 'rutas'}
                    </span>{' '}
                    desde {hubName}. Destino más lejano{' '}
                    <span className="text-ink" style={{ fontWeight: 500 }}>
                      {longest?.trip.end_location}
                    </span>{' '}
                    a <span className="text-ink" style={{ fontWeight: 500 }}>
                      {nf0.format(longest?.km ?? 0)} km
                    </span>.
                  </>
                ) : (
            ...
            <div className="flex items-center justify-between flex-wrap" style={{ gap: 12 }}>
              ...
              <div className="flex items-center" style={{ gap: 6 }}>
                <span
                  className="font-mono uppercase"
                  style={{
                    fontSize: 10, letterSpacing: '.18em', color: '#a1a1a6', marginRight: 2,
                  }}
                >
                  Filtros
                </span>
                {(Object.keys(FILTER_LABEL) as FilterKey[]).map((f) => (
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${filter === f ? '#1d1d1f' : 'var(--color-silver-mist)'}`,
            ...
            {!hasTrips ? (
              ...
              <div
                style={{
                  ...
                  justifyContent: 'center', textAlign: 'center', padding: '56px 20px', gap: 6,
                }}
              >
                <Route size={36} strokeWidth={1.4} style={{ color: '#c7c7cc' }} />
                <p className="text-ink" style={{ fontSize: 16, fontWeight: 600, marginTop: 6 }}>
                ...
                <button
                  ...
                  className="transition-opacity hover:opacity-85"
                  style={{
                    marginTop: 12, background: '#1d1d1f', color: '#fff', borderRadius: 999,
                    border: 'none', padding: '10px 18px', fontWeight: 500, fontSize: 13,
                    cursor: 'pointer',
                  }}
                >
              ...
              <>
                ...
                <div
                  className="grid"
                  style={{
                    gridTemplateColumns: '32px 1.4fr 1.3fr 56px 50px 60px',
                    gap: 14, padding: '8px 0', marginTop: 14,
                ...
                >
                  {['#', 'Ruta · fecha', 'Distancia', 'Min', 'L', 'Km/h'].map((h, i) => (
                    <span
                      ...
                      className="font-mono uppercase"
                      style={{
                        fontSize: 10, fontWeight: 500, letterSpacing: '.18em', color: '#a1a1a6',
                ...
                {visibleRows.length === 0 ? (
                  <div style={{ padding: '40px 0', textAlign: 'center' }}>
                    <p style={{ fontSize: 14, color: '#707070' }}>
                      Sin viajes en «{FILTER_LABEL[filter]}».
                    </p>
                    <button
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            borderTop: i === 0 ? 'none' : '1px solid var(--color-silver-mist)',
                            cursor: 'pointer', position: 'relative',
                            background: isSel ? 'var(--color-fog)' : 'transparent',
                          }}
                        >
                          {/* # */}
                          <span
                            className="font-mono"
                            style={{
                              fontSize: 13, fontWeight: 500, color: '#a1a1a6',
                              letterSpacing: '.06em',
                            }}
                          >
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            >
                              <span
                                style={{
                                  ...
                                  color: isMax ? '#b64400' : '#1d1d1f',
                                }}
                              >
                                {r.trip.start_location}
                                <span style={{ fontWeight: 300, color: '#a1a1a6', margin: '0 6px' }}>
                                  →
                                </span>
                                {r.trip.end_location}
                              </span>
                            ...
                            <p
                              className="font-mono"
                              style={{
                                fontSize: 11, fontWeight: 500, color: '#a1a1a6',
                          ...
                          <div className="flex items-center" style={{ gap: 10, minWidth: 0 }}>
                            <div
                              style={{
                                flex: 1, height: 8, background: '#f5f5f7',
                                borderRadius: 999, overflow: 'hidden',
                              }}
                            >
                              <div
                                style={{
                                  width: `${(r.km / maxKm) * 100}%`, height: '100%',
                                  background: isMax ? '#b64400' : '#1d1d1f', borderRadius: 999,
                                }}
                              />
                            </div>
                            <span
                              ...
                              style={{
                                fontFamily: 'Inter, var(--font-sf-pro-display)',
                                fontWeight: 700, fontSize: isMax ? 17 : 15,
                                letterSpacing: '-0.2px', color: isMax ? '#b64400' : '#1d1d1f',
                          ...
                          {/* Min / L / Km/h */}
                          {[
                            nf0.format(r.minutes),
                            r.fuelL > 0 ? nf1.format(r.fuelL) : '—',
                            r.avgKmh > 0 ? nf0.format(r.avgKmh) : '—',
                          ].map((val, j) => (
                            <span
                              key={j}
                              className="font-mono tabular-nums"
                              style={{
                                fontSize: 13, fontWeight: 500, color: '#707070',
                          ...
                          {/* Eliminar — visible al pasar el cursor */}
                          <button
                            onClick={(e) => { e.stopPropagation(); handleDelete(r.trip.id); }}
                            ...
                            style={{
                              position: 'absolute', top: 10, right: -6, width: 22, height: 22,
                              borderRadius: 999, border: 'none', background: 'var(--color-snow)',
                ...
                {visibleRows.length > 0 && (
                  <div
                    ...
                    style={{
                      gridTemplateColumns: '32px 1.4fr 1.3fr 56px 50px 60px',
                      gap: 14, marginTop: 'auto', paddingTop: 14,
                      borderTop: '1px solid #1d1d1f',
                    }}
                  >
                    <span
                      className="font-mono"
                      style={{ fontSize: 10, color: '#a1a1a6', letterSpacing: '.14em' }}
                    >
                      Σ
                    </span>
                    <span
                      className="font-mono uppercase"
                      style={{ fontSize: 12, color: '#707070', letterSpacing: '.06em' }}
                    >
                    ...
                    {[
                      nf0.format(visMin),
                      visFuel > 0 ? nf1.format(visFuel) : '—',
                      visMin > 0 ? nf0.format(visKmh) : '—',
                    ].map((val, j) => (
                      <span
                        key={j}
                        className="font-mono tabular-nums"
                        style={{ fontSize: 13, fontWeight: 500, color: '#707070', textAlign: 'right' }}
                      >
                        {val}
                      </span>
                    ))}
