...
export const TripsPage = () => {
  ...
  } = useTripChecklist(selectedId ?? undefined);
  ...
  }, [selectedId, fetchBookings, fetchChecklist]);
  ...
  /* ─── Datos agregados ───────────────────────────────────────────────────── */
  const stats = useMemo(() => {
    const list: Row[] = [...registeredTrips]
  ...
  useEffect(() => {
    if (statusFilter === 'planning') {
      ...
      setSelectedId((prev) =>
        prev && planningTrips.some((t) => t.id === prev) ? prev : planningTrips[0].id,
      );
      return;
    }
    if (!stats.longest) {
    ...
    setSelectedId((prev) =>
      prev && registeredTrips.some((t) => t.id === prev) ? prev : stats.longest!.trip.id,
    );
  }, [trips, statusFilter, planningTrips, registeredTrips, stats.longest]);
  ...
  const periodDate =
    hasTrips && stats.list[0]?.trip.start_datetime
      ? parseISO(stats.list[0].trip.start_datetime)
      : new Date();
  ...
  const handleCreateDraft = async (data: CreateDraftTripInput) => {
    const draft = await createDraft(user.id, data);
    setStatusFilter('planning');
    setSelectedId(draft.id);
    toast.success('Borrador creado');
  };

  const handleConfirmTrip = async (keepIds: string[]) => {
    if (!selectedId) return;
    await confirmTrip(selectedId, keepIds);
    await fetchBookings();
  ...
  const handleAddChecklist = async (text: string) => {
    await addChecklistItem(user.id, text);
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* ─── Switch: Registrados / En planificación ─────────────────── */}
        <div className="flex items-center" style={{ gap: 8 }}>
          {(['registered', 'planning'] as const).map((k) => {
            const active = statusFilter === k;
            const count = k === 'planning' ? planningTrips.length : registeredTrips.length;
            return (
              <button
                key={k}
                ...
                style={{
                  ...
                  fontSize: 13,
                  fontWeight: 500,
                  border: `1px solid ${active ? '#1d1d1f' : 'var(--color-silver-mist)'}`,
              ...
              >
                {k === 'planning' ? 'En planificación' : 'Registrados'}
                <span
                  className="tabular-nums"
                  style={{
                    fontSize: 11,
                    opacity: 0.7,
                    background: active ? 'rgba(255,255,255,.18)' : 'var(--color-fog)',
                    padding: '1px 7px',
                    borderRadius: 999,
                  }}
                >
              ...
              <div>
                ...
                >
                  {hasTrips ? (
                    ...
                    <>
                      Aún no has registrado ningún viaje. Empieza a documentar tus rutas, consumo y
                      kilómetros.
                    </>
                  )}
                </p>
                  ...
                  {(['corto', 'medio', 'largo'] as TagKind[]).map((tag, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center" style={{ gap: 8 }}>
                          <TripTag tag={tag} />
                          <span className="font-mono" style={{ fontSize: 11, color: '#707070' }}>
                            {d.count}
                          </span>
                        </div>
                        <div
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    ...
                    style={{ fontSize: 10, letterSpacing: '.12em', color: '#c7c7cc' }}
                  >
                ...
                >
                  {!MAPBOX_TOKEN ? (
                    ...
                    >
                      ...
                      <p
                        className="font-mono"
                        style={{ fontSize: 11, color: '#a1a1a6', letterSpacing: '.06em' }}
                      >
                    ...
                    >
                        ...
                        <Marker longitude={hubCoords.lng} latitude={hubCoords.lat} anchor="center">
                          <div
                            style={{
                              ...
                              boxShadow: '0 0 0 3px rgba(29,29,31,.25)',
                            }}
                          />
                        </Marker>
                      )}
                      ...
                      {destMarkers.map((r) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              title={`${r.trip.end_location} · ${nf0.format(r.km)} km`}
                              style={{
                                width: isSel ? 14 : 10,
                                height: isSel ? 14 : 10,
                                borderRadius: '50%',
                                background: isMax ? '#b64400' : '#707070',
                                border: '2px solid #fff',
                                boxShadow: isSel
                                  ? `0 0 0 3px ${isMax ? '#b64400' : '#1d1d1f'}`
                                  : 'none',
                                cursor: 'pointer',
                      ...
                      {!hasCoords && (
                        ...
                        >
                          ...
                          >
                            Registra viajes con coordenadas GPS
                            <br />
                            para ver las rutas en el mapa
                          </p>
                        </div>
                      )}
                    </MapGL>
              ...
              >
                ...
                {!hasTrips ? (
                  ...
                  >
                    ...
                    <p
                      style={{ fontSize: 13, color: '#707070', maxWidth: 320, textWrap: 'pretty' }}
                    >
                            ...
                            >
                              ...
                              <div style={{ minWidth: 0 }}>
                                <div className="flex items-baseline flex-wrap" style={{ gap: 8 }}>
                                  ...
                                  >
                                    ...
                                    <span
                                      style={{ fontWeight: 300, color: '#a1a1a6', margin: '0 6px' }}
                                    >
                                      →
                                    </span>
                                    {r.trip.end_location}
                              ...
                              <div className="flex items-center" style={{ gap: 10, minWidth: 0 }}>
                                ...
                                >
                                  <div
                                    style={{
                                      width: `${(r.km / maxKm) * 100}%`,
                                      height: '100%',
                                      background: isMax ? '#b64400' : '#1d1d1f',
                                      borderRadius: 999,
                                    }}
                                  />
                                </div>
                                ...
                                <span style={{ fontSize: 10, color: '#a1a1a6', marginLeft: 2 }}>
                                  km
                                </span>
                              </div>
                              {[
            ...
            {selectedTrip && (
              ...
              >
                <div className="flex items-center justify-between flex-wrap" style={{ gap: 12 }}>
                  <div>
                    <span style={mistMono}>
                      § Reservas · {selectedTrip.start_location} → {selectedTrip.end_location}
                    </span>
                    <p className="text-graphite" style={{ fontSize: 13, marginTop: 4 }}>
                      Alojamientos, museos, actividades y entradas de este viaje.
                    </p>
                  </div>
                  <button
                ...
                {bookings.length === 0 ? (
                  <p style={{ marginTop: 18, fontSize: 13, color: '#707070' }}>
                    Sin reservas registradas para este viaje.
                  </p>
                ) : (
      ...
      {showQuickPlan && (
        <QuickPlanTripForm onSubmit={handleCreateDraft} onClose={() => setShowQuickPlan(false)} />
      )}
      {showForm && (
        <TripForm
