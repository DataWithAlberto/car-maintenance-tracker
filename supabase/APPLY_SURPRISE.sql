-- ╔══════════════════════════════════════════════════════════════════════╗
-- ║  SCRIPT CONSOLIDADO · Viaje sorpresa                                    ║
-- ║  Ejecuta TODO esto en Supabase → SQL Editor → Run.                     ║
-- ║  Es idempotente: puedes ejecutarlo las veces que quieras.              ║
-- ║                                                                        ║
-- ║  NOTA: los campos nuevos (music_url, spotify_url, route_stops,         ║
-- ║  cover_url, audio_url, reason, hints, fun_facts, opened_at) viven       ║
-- ║  dentro de surprise_config (JSONB), así que NO hace falta ALTER TABLE. ║
-- ║  Solo se actualizan estas 3 funciones RPC.                             ║
-- ╚══════════════════════════════════════════════════════════════════════╝

-- ─── 1. get_public_trip ─────────────────────────────────────────────────────
-- Devuelve el viaje por share_token. Accesible si es público O is_surprise.
-- En estado bloqueado expone cover_url y las pistas reveladas según calendario.
...
BEGIN
  ...
  IF v_trip.is_surprise
    ...
    IF v_total_hints > 0 THEN
      v_days_until := GREATEST(1, EXTRACT(DAY FROM (v_now - v_trip.created_at))::INTEGER + 1);
  ...
  SELECT COALESCE(
           jsonb_agg(
             jsonb_build_object(
               'id', g.id, 'public_url', g.public_url, 'caption', g.caption,
               'taken_at', g.taken_at, 'latitude', g.latitude, 'longitude', g.longitude,
               'width', g.width, 'height', g.height, 'activity_id', g.activity_id,
...
-- ─── 2. mark_surprise_opened ────────────────────────────────────────────────
-- Registra opened_at la 1ª vez que el destinatario abre la sorpresa.
...
BEGIN
  SELECT * INTO v_trip FROM trips
   WHERE share_token = p_token AND is_surprise = true;
  ...
  IF (v_trip.surprise_config->>'opened_at') IS NULL THEN
    UPDATE trips
       SET surprise_config = jsonb_set(
             ...
             '{opened_at}', to_jsonb(NOW()::text), true)
     WHERE id = v_trip.id;
  END IF;
END;
$$;
...
-- ─── 3. add_surprise_reaction ───────────────────────────────────────────────
-- Suma +1 a una reacción emoji. Solo acepta los 4 emojis previstos.
CREATE OR REPLACE FUNCTION public.add_surprise_reaction(p_token TEXT, p_emoji TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
